...
async function startServer() {
  ...
  app.post("/api/listings", async (req: Request, res: Response) => {
    try {
      ...
      if (
        !food_item ||
        typeof food_item !== "string" ||
        food_item.trim().length < 2
      ) {
        return res
          .status(400)
          .json({
            error: "Food item description is required (at least 2 characters).",
          });
      }
      if (
        !quantity ||
        ...
        return res
          .status(400)
          .json({
            error: 'Quantity is required (e.g. "3 portions", "1 tray").',
          });
      }
      if (
        !location ||
        ...
        return res
          .status(400)
          .json({ error: "Pickup location address is required." });
      }
      if (!available_from || typeof available_from !== "string") {
        return res
          .status(400)
          .json({ error: "Available start time is required." });
      }
      if (!available_until || typeof available_until !== "string") {
        return res
          .status(400)
          .json({ error: "Available end time is required." });
      }
      if (
        !contact_info ||
        typeof contact_info !== "string" ||
        contact_info.trim().length < 2
      ) {
        return res
          .status(400)
          .json({ error: "Contact phone or name is required." });
      }
      ...
      return res.status(500).json({
        ...
        details: err?.message || "Unknown Firestore error",
      });
    }
  });
